...
pub async fn detail(id: i32)-> std::result::Result<impl Reply, Rejection>  {
    ...
    let article_content = article_content_model::get_article_content(article.content_id);
    ...
    let html = to_html_base("article_detail.html", data);
...
pub async fn list(page: u32) -> std::result::Result<impl Reply, Rejection> {
    ...
    let html = to_html_base("article_list.html", data);
    Ok(warp::reply::html(html))
}
